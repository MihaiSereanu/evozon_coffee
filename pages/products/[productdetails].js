                ...
                <div className={styles.listWrapper}>
                  <ListGroup variant='flush' className={styles.recipeList}>
                    {recipe.map((ingredient, idx) => {
                      return <ListGroup.Item key={idx} style={{ backgroundColor: '#E4D9C5', border: 'none' }}>{ingredient}</ListGroup.Item>
                    })}
                  </ListGroup>
                  <Button variant='dark' className={styles.addBtn} onClick={() => addPredefined(product)}>Add to cart</Button>
                </div>
                <Image src='http://t0.gstatic.com/images?q=tbn:ANd9GcTQhEuCyNU_2t8HXuGGSAODNGQhigx21H0VbVWPjMG3qouOOafTZgOaOMoFKsTyBkasAQrvTH4Iu8YTiQmbH_o' fluid className={styles.coffeeImg} />
              </div>
            </Container>
